...
namespace OpenIdDictMvcLib.Areas.Identity.Pages.Account
{
    public class LoginWith2faModel : PageModel
    {
        ...
        public async Task<IActionResult> OnPostAsync(bool rememberMe, string returnUrl = null)
        {
            ...
            if (result.Succeeded)
            {
                string s1 = "User with ID '{UserId}' logged in with 2fa.";
                _logger.LogInformation(s1, user.Id);
                return LocalRedirect(returnUrl);
            }
            else if (result.IsLockedOut)
            {
                string s2 = "User with ID '{UserId}' account locked out.";
                _logger.LogWarning(s2, user.Id);
                return RedirectToPage("./Lockout");
            }
            else
            {
                string s3 = "Invalid authenticator code entered for user with ID '{UserId}'.";
                _logger.LogWarning(s3, user.Id);
                string s4 = _sharedLocalizer["Invalid authenticator code."];
