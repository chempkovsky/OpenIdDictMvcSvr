...
namespace OpenIdDictMvcLib.Areas.Identity.Pages.Account.Manage
{
    public class ExternalLoginsModel : PageModel
    {
        ...
        public async Task<IActionResult> OnGetAsync()
        {
            ...
            if (_userStore is IUserPasswordStore<OidcIdentityUser> userPasswordStore)
            {
        ...
        public async Task<IActionResult> OnPostRemoveLoginAsync(string loginProvider, string providerKey)
        {
            ...
            if (!result.Succeeded)
            {
                StatusMessage = _sharedLocalizer["The external login was not removed."];
                return RedirectToPage();
            }

            await _signInManager.RefreshSignInAsync(user);
            StatusMessage = _sharedLocalizer["The external login was removed."];
            return RedirectToPage();
        }
        ...
        public async Task<IActionResult> OnGetLinkLoginCallbackAsync()
        {
            ...
            if (info == null)
            {
                string s1 = _sharedLocalizer[$"Unexpected error occurred loading external login info."];
            ...
            if (!result.Succeeded)
            {
                StatusMessage = _sharedLocalizer["The external login was not added. External logins can only be associated with one account."];
                return RedirectToPage();
            }
            ...
            StatusMessage = _sharedLocalizer["The external login was added."];
            return RedirectToPage();
        }
    }
}
